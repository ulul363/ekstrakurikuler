...
@section('content')
    ...
    <script>
        function createChatMessage(chat, currentUser) {
            ...
            var avatarSrc = userRole === 'Pembina' ? '{{ asset('assets/images/profile/bg-4.jpg') }}' : (userRole === 'Ketua' ? '{{ asset('assets/images/profile/bg-3.jpg') }}' : '{{ asset('assets/images/profile/bg-4.jpg') }}');
        ...
        function fetchMessages() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var wasScrolledToBottom = chatContainer.scrollTop() + chatContainer.height() >= chatContainer.prop('scrollHeight');
            ...
            $('#chat-form').submit(function(event) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        var chatContainer = $('#chat-isi');
                        chatContainer.append(createChatMessage(response, {{ auth()->user()->id }}));
                        chatContainer.scrollTop(chatContainer.prop('scrollHeight'));
